 /**Törli a propjectből a megadott felhasználókat amennyiben a project létezik és van jogunk módosítani a projectet */
	...
	return function (req, res, next) {
		...
		projektModel.findById(req.query.id, function (err, result) {
            ...
            for(i in req.body.users){
				for(j in result.users){
					if(req.body.users[i] === result.users[j]){
						result.users.splice(j,1);
						break;
					}
				}
			}
        	if(result.save(function (err) {
  				...
  				if (err) {
					...
					logger(4,req.body.users + " removed from project ("+ req.query.id + ")");
					return true;
  				}
			})){
